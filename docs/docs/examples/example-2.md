...
library(tidyr)
summarized_results %>%
  select(estimator, n, mean_se_beta0, mean_se_beta1) %>%
  pivot_longer(
...
summarized_results %>%
  select(estimator, n, cov_beta0, cov_beta1) %>%
  pivot_longer(

...
# Add the creator function to the simulation object
...
# Add the function to the simulation object
sim %<>% add_method(est_tx_effect)
```
...
The simulation script is a function that runs a single simulation replicate and returns the results. Within a script, you can reference the current simulation level values using the variable *L*. For example, when the first simulation replicate is running, `L$estimator` will equal "est1" and `L$num_patients` will equal 50. In the last simulation replicate, `L$estimator` will equal "est2" and `L$num_patients` will equal 1,000. Your script will have access to any creators and methods that have been added to your simulation object.
...
```R
sim %>% summarize(
  bias = list(name="bias_est", truth=-7, estimate="est"),
  mse = list(name="mse_est", truth=-7, estimate="est")
)
#>   level_id estimator num_patients mean_runtime   mean_est    bias_est      mse_est
#> 1        1     est_1           50  0.004853940  -4.086867  2.91313267 1221.3380496
#> 2        2     est_2           50  0.004429979  -7.116263 -0.11626316    1.9830060
#> 3        3     est_1          100  0.008662744 -12.559415 -5.55941452  614.2357481
#> 4        4     est_2          100  0.008607748  -7.057722 -0.05772176    0.9293629
#> 5        5     est_1          200  0.018162758  -6.771745  0.22825547  376.3410262
#> 6        6     est_2          200  0.016390090  -6.795681  0.20431877    0.5332393
```

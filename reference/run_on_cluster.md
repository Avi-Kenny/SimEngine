...
This function allows for simulations to be run in parallel on a cluster
computing system (CCS). See the
...
SimEngine. `run_on_cluster` acts as a wrapper for the code in your
...
summarize simulation results). This function is to be used in
...
the CCS.
[`run`](https://avi-kenny.github.io/SimEngine/reference/run.md))), and
"last" (usually code to process or summarize simulation results). This
function interacts with cluster job scheduler software (e.g. Slurm or
Oracle Grid Engine) to divide parallel tasks over cluster nodes.
...
- first:
  ...
  code enclosed by curly braces that creates and sets up a simulation
  object.
...
- last:
  ...
  simulation object (which at this point will contain your results),
